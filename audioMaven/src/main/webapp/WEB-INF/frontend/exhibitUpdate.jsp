...
<body>
	...
	<div class="container">
		<form:form action="${pageContext.request.contextPath}/exhibitUpdate/${exhibit.exhibitId}"
			...
			<p class="title">Ažuriranje izloška</p>
			<input type="text" name="name" placeholder="Name of exhibit" value="${exhibit.name}" autofocus required />
			<textarea name="description" placeholder="Description of exhibit"
				cols="79" rows="5" required>${exhibit.description}</textarea>
			
			<p><strong>Slika izloška: </strong> ${exhibit.image_path} <form:input class="imageUpload" type="file"  path="files[0].file" id="files[0].file"/></p>
			
			<p><strong>Audio zapis izloška: </strong> ${exhibit.audio_path}<form:input class="audioUpload" type="file" path="files[1].file" id="files[1].file"/></p>
			
			<button type="submit" name="Upload">
       			 <i class="spinner"></i>
        		<span class="state">Ažuriraj izložak</span>
     		 </button>
