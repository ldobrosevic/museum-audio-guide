...
<body>
	...
	<div class="container">
		<form:form action="${pageContext.request.contextPath}/exhibitionUpdate/${exhibition.exhibitionId}"
			...
			<p class="title">Ažuriranje izložbe</p>
			<input type="text" name="name" placeholder="Name of exhibitions" value="${exhibition.nameOfExhibition}" autofocus required />
			<textarea name="description" placeholder="Description of exhibition"
				cols="79" rows="5" required>${exhibition.description}</textarea>
			
			<p><strong>Datum početka:</strong> <input class="imageUpload" name="begin" type="date" value="${begin}" required></p>
			<p><strong>Datum završetka:</strong> <input class="imageUpload" name="end" type="date" value="${end}" required></p>
			
			<p><strong>Vrsta izložbe: </strong>
			<select class="audioUpload" name="chosen">
			...
			<button type="submit" name="Upload">
       			 <i class="spinner"></i>
        		<span class="state">Ažuriraj izložbu</span>
     		 </button>
